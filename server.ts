...
// POST /api/extract-health - Extract structured health data from voice call transcript
app.post('/api/extract-health', async (req: Request, res: Response) => {
...
Return strictly a valid JSON object matching this schema:
{
  ...
  for (const model of candidateModels) {
    try {
      ...
      console.warn(`[Server /api/extract-health] Model ${model} failed:`, e.message);
    }
  }

  res.status(500).json({ error: 'All Gemini models failed extraction' });
});
