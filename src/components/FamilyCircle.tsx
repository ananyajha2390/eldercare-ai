                ...
                >
                  ...
                  <div className="text-xs text-slate-500 space-y-1.5 mb-4">
                    ...
                    {member.location && (
                      <div className="flex justify-between items-center">
                        <span>Location:</span>
                        <span className="text-slate-700 truncate max-w-[140px]">{member.location}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-slate-200/60 space-y-2">
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => handleSimulateCall(member)}
                      className="flex-1 py-2 rounded-xl text-xs font-bold bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveModal({ type: 'message', member })}
                      className="flex-1 py-2 rounded-xl text-xs font-bold bg-teal-600 hover:bg-teal-700 text-white flex items-center justify-center gap-1.5 transition-colors shadow-xs cursor-pointer"
                    >
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 animate-in zoom-in-95">
            ...
            <form onSubmit={handleSendMessage} className="space-y-4">
              <textarea
                ...
                value={messageInput}
                onChange={(e) => setMessageInput(e.target.value)}
                placeholder={`Hi ${activeModal.member.name.split(' ')[0]}, here is a quick note...`}
                className="w-full p-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2 rounded-xl text-xs font-bold bg-teal-600 text-white hover:bg-teal-700 flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
                >
                  <Send className="w-3.5 h-3.5" />
                  <span>Send</span>
                </button>
